...
function main {
  ...
  for file in ${DATA}/${DIRECTORY}/${DATA_GENE_DIR}/*; do if [[ -f "${file}" ]]; then rm "${file}" >> "${LOG}" 2>&1; fi; done
  for file in ${DATA}/${DIRECTORY}/${DATA_PROT_DIR}/*; do if [[ -f "${file}" ]]; then rm "${file}" >> "${LOG}" 2>&1; fi; done
  ...
  if [[ ! "$(ls -A ${DATA}/${DIRECTORY}/${DATA_GENE_DIR})" ]]; then echo "[ERROR] directory ${DATA}/${DIRECTORY}/${DATA_GENE_DIR} is empty, but required for the next step" >> "${LOG}" 2>&1; exit 1; fi
  if [[ ! "$(ls -A ${DATA}/${DIRECTORY}/${DATA_PROT_DIR})" ]]; then echo "[ERROR] directory ${DATA}/${DIRECTORY}/${DATA_PROT_DIR} is empty, but required for the next step" >> "${LOG}" 2>&1; exit 1; fi

  #Convert current fasta format to gembase format
  ...
  mv "${DATA_ECAMBER_OUT}/${DIRECTORY}/anns_input" "${DATA_ECAMBER_OUT}/${DIRECTORY}/anns_parsed" "${DATA_ECAMBER_OUT}/${DIRECTORY}/blast" "${DATA_ECAMBER_OUT}/${DIRECTORY}/cambervis" "${DATA_ECAMBER_OUT}/${DIRECTORY}/formatdb.log" "${DATA_ECAMBER_OUT}/${DIRECTORY}/genomes" "${DATA_ECAMBER_OUT}/${DIRECTORY}/genomes_dbs" "${DATA_ECAMBER_OUT}/${DIRECTORY}/output" "${DATA_ECAMBER_OUT}/${DIRECTORY}/prodigal" "${DATA_ECAMBER_OUT}/${DIRECTORY}/results" "${DATA_ECAMBER_OUT}/${DIRECTORY}/strains.txt" "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/."  >> "$LOG" 2>&1

  #Delete eCamber outputs (previously copied)
  if [[ -d "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/anns_input" ]]; then rm -r "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/anns_input"; fi
  if [[ -d "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/anns_parsed" ]]; then rm -r "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/anns_parsed"; fi
  if [[ -d "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/blast" ]]; then rm -r "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/blast"; fi
  if [[ -d "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/cambervis" ]]; then rm -r "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/cambervis"; fi
  if [[ -d "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/genomes" ]]; then rm -r "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/genomes"; fi
  if [[ -d "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/genomes_dbs" ]]; then rm -r "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/genomes_dbs"; fi
  if [[ -d "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/output" ]]; then rm -r "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/output"; fi
  if [[ -d "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/prodigal" ]]; then rm -r "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/prodigal"; fi
  if [[ -d "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/results" ]]; then rm -r "${DATA}/${DIRECTORY}/${DATA_ECAMBER_OUTPUT}/results"; fi
