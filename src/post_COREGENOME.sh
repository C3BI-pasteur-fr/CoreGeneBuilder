...
#                                                                                                                   #
# post_COREGENOME.sh: steps after building a core or persistent gene set.                                           #
...
function main {
  ...
  LOG="${DATA}/${DIRECTORY}/logs/${DIRECTORY}.coregenome.3.log";
  readonly LOG;
  echo '' > "${LOG}";
  ...
  #Copy of coreGenome outputs
  echo '...core genome building is over' >> "${LOG}" 2>&1
  echo '...core genome building is over' ;
  echo "copy core genome output to main directory : ${DATA}/${DIRECTORY}/${DATA_CORE_DIR}" >> "${LOG}" 2>&1
  echo "copy core genome output to main directory : ${DATA}/${DIRECTORY}/${DATA_CORE_DIR}";
  cp -p "${DATA}/${DIRECTORY}/${DATA_CORE_DIR}/${output_dir}/second" "${DATA}/${DIRECTORY}/${DATA_CORE_DIR}/${output_dir}/CoreGenome-${DIRECTORY}.lst" "${DATA}/${DIRECTORY}/${DATA_CORE_DIR}" >> "${LOG}" 2>&1

  z=""; boolean="$(echo "${CGGENOMEPRCT} < 100.0" |bc -l)"; if [[ $boolean -eq 1 ]]; then z='0'; fi
  listcoregenes="${DATA}/${DIRECTORY}/${DATA_CORE_DIR}/${output_dir}/CoreGenome-${DIRECTORY}-${z}${CGGENOMEPRCT}.lst"
  if [[ ! -e "${listcoregenes}" ]]; then echo "[ERROR] the input file ${listcoregenes} does not exist :: No core genes found, restart program with new parameters !" 2>>"${LOG}"; exit 1; fi
  if [[ ! -s "${listcoregenes}" ]]; then echo "[ERROR] the input file ${listcoregenes} is empty :: No core genes found, restart program with new parameters !" 2>>"${LOG}"; exit 1; fi
  cp -p "${listcoregenes}" "${DATA}/${DIRECTORY}/${DATA_CORE_DIR}" >> "${LOG}" 2>&1
  ...
  rm "${DATA}/${DIRECTORY}/logs/${DIRECTORY}.coregenome."?.log;
}
